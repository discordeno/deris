...
import { API_VERSION } from "discordeno/*";
...
export class Shard extends EventEmitter {
  ...
  onPacket(packet: DiscordGatewayPayload) {
    ...
    switch (packet.op) {
      ...
      case GatewayOpcodes.Hello: {
        if ((packet.d as DiscordHello).heartbeat_interval > 0) {
          ...
          this.heartbeatInterval = setInterval(
            () => this.heartbeat(true),
            (packet.d as DiscordHello).heartbeat_interval
          );
        }
      ...
      case GatewayOpcodes.HeartbeatACK: {
        ...
        this.latency =
          this.lastHeartbeatReceived - (this.lastHeartbeatSent || 0);
        break;
      }
      default: {
  ...
  requestGuildMembers(guildID: string, options?: RequestGuildMembersOptions) {
    ...
    if (
      ...
      !(this.client.options.intents & Intents.GuildMembers)
    ) {
    ...
    if (
      opts.presences &&
      this.client.options.intents &&
      !(this.client.options.intents & Intents.GuildPresences)
    ) {
      throw new Error(
    ...
    switch (
        ...
        if (packet.d.user.username !== undefined) {
          ...
          if (!user || oldUser) {
            user = this.client.users.update(
              new User(packet.d.user, this.client),
              this.client
            );
      ...
      case "VOICE_STATE_UPDATE": {
        ...
        if (oldChannelID != packet.d.channel_id) {
          let oldChannel: TextVoiceChannel | StageChannel | null,
            newChannel: TextVoiceChannel | StageChannel | null;
          if (oldChannelID) {
            oldChannel = guild.channels.get(oldChannelID) as
              | TextVoiceChannel
              | StageChannel;
            if (
              oldChannel &&
              oldChannel.type !== ChannelTypes.GuildVoice &&
              oldChannel.type !== ChannelTypes.GuildStageVoice
            ) {
              this.emit(
                "warn",
          ...
          if (
            packet.d.channel_id &&
            (newChannel = guild.channels.get(packet.d.channel_id) as
              | TextVoiceChannel
              | StageChannel) &&
            (newChannel.type === ChannelTypes.GuildVoice ||
              newChannel.type === ChannelTypes.GuildStageVoice)
          ) {
      ...
      case "MESSAGE_UPDATE": {
        ...
        this.emit(
          "messageUpdate",
          channel.messages.update(new Message(packet.d, this.client)),
          oldMessage
        );
        break;
      }
          ...
          if ((channel as GuildTextableChannel).guild) {
            if (packet.d.member) {
              ...
              member = channel.guild.members.update(
                packet.d.member,
                (channel as GuildTextableChannel).guild
              );
            }
          }
        }
      ...
      case "GUILD_MEMBER_ADD": {
        ...
        this.emit(
          "guildMemberAdd",
          guild,
          guild.members.add(new Member(packet.d, guild, this.client))
        );
        break;
      }
      ...
      case "GUILD_BAN_ADD": {
        ...
        this.emit(
          ...
          this.client.users.update(new User(packet.d.user, this.client))
        );
        break;
      }
      ...
      case "GUILD_ROLE_CREATE": {
        ...
        this.emit(
          "guildRoleCreate",
          guild,
          guild.roles.add(new Role(packet.d.role, guild))
        );
        break;
      }
      case "GUILD_ROLE_UPDATE": {
        ...
        this.emit(
          "guildRoleUpdate",
          guild,
          guild.roles.update(new Role(packet.d.role, guild)),
          oldRole
        );
        break;
      }
        ...
        if (oldType === packet.d.type) {
          ...
          if (packet.d.guild_id) {
            ...
            this.client.privateChannels.add(
              newChannel as unknown as PrivateChannel,
              this.client
            );
          } else {
            this.emit(
        ...
        if (packet.d.type === ChannelTypes.DM || packet.d.type === undefined) {
          if (this.id === 0) {
            const channel = this.client.privateChannels.remove(
              new PrivateChannel(packet.d, this.client)
            );
            if (channel) {
          ...
          const channel = guild.channels.remove(
            new GuildChannel(packet.d, this.client)
          );
          if (!channel) {
            break;
          }
      ...
      case "READY": {
        ...
        packet.d.guilds.forEach((guild) => {
          if (guild.unavailable) {
            ...
            this.client.unavailableGuilds.set(
              guild.id,
  ...
  _onWSClose(code: number, reason: string) {
    ...
    let err: (Error & { code?: number }) | null =
