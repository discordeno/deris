...
export class Client extends EventEmitter {
  ...
  async addMessageReaction(
    ...
    return await this.put(
      CHANNEL_MESSAGE_REACTION_USER(channelID, messageID, reaction, "@me")
    );
  }
  ...
  async createGuildFromTemplate(
    ...
    return await this.post(GUILD_TEMPLATE(code), { body: { name, icon } }).then(
      (guild) => new Guild(guild, this)
    );
  }
  ...
  async createInteractionResponse(
    ...
    return await this.post(
      INTERACTION_RESPOND(interactionID, interactionToken),
      {
        body: {
          ...options,
          data: {
  ...
  async crosspostMessage(
    ...
    return await this.post(CHANNEL_CROSSPOST(channelID, messageID)).then(
  ...
  async deleteGuildCommand(
    ...
    return await this.delete(
      GUILD_COMMAND(this.applicationId, guildID, commandID)
    );
  }

  /** Delete a guild discovery subcategory */
  async deleteGuildDiscoverySubcategory(
    guildID: BigString,
    categoryID: BigString,
    reason?: string
  ) {
  ...
  async deleteWebhook(
    ...
    return await this.delete(
      token ? WEBHOOK_TOKEN(webhookID, token) : WEBHOOK(webhookID),
      {
        reason,
      }
    );
  ...
  async editChannelPosition(
    ...
    return this.patch(GUILD_CHANNELS(guild.id), {
  ...
  async editCommandPermissions(
    ...
    return this.put(
      COMMAND_PERMISSIONS(this.applicationId, guildID, commandID),
      {
        body: permissions,
      }
    );
  ...
  async editSelf(options: {
    ...
    return await this.patch(USER("@me"), { body: { ...options } }).then(
      (data) => new ExtendedUser(data, this)
    );
  }
  ...
  async executeWebhook(
    ...
    return await this.post(
      WEBHOOK_TOKEN(webhookID, token) + (qs ? "?" + qs : ""),
      {
        body: {
    ...
    return await this.get(THREADS_GUILD_ACTIVE(guildID)).then((response) => {
      return {
        members: response.members.map(
          (member: DiscordThreadMember) => new ThreadMember(member, this)
        ),
        threads: response.threads.map((thread: DiscordChannel) =>
  ...
  async getArchivedThreads(
    ...
    return await this.get(
      THREADS_ARCHIVED(channelID, type) + (qs ? "?" + qs : "")
    ).then((response) => {
      return {
  ...
  getChannel(channelID: BigString): AnyChannel {
    ...
    const guildID =
      this._channelGuildMap.get(channelID) ??
      this._threadGuildMap.get(channelID);
  ...
  async getDMChannel(userID: BigString): Promise<PrivateChannel> {
    if (this._privateChannelMap.has(userID)) {
      return Promise.resolve(
        this.privateChannels.get(this._privateChannelMap.get(userID)!)!
      );
    }
  ...
  async getEmojiGuild(emojiID: BigString): Promise<Guild> {
    return await this.get(CUSTOM_EMOJI_GUILD(emojiID)).then(
      (result) => new Guild(result, this)
    );
  }
  ...
  async getGuildAuditLog(
    ...
    return await this.get(
      GUILD_AUDIT_LOGS(guildID) + (qs ? "?" + qs : "")
    ).then((data) => {
      const guild = this.guilds.get(guildID);
      ...
      return {
        entries: data.audit_log_entries.map(
          (entry: DiscordAuditLogEntry) => new GuildAuditLogEntry(entry, guild!)
        ),
  ...
  async getGuildCommandPermissions(
    ...
    return await this.get(
      GUILD_COMMAND_PERMISSIONS(this.applicationId, guildID)
    );
  }
  ...
  async getGuildPreview(guildID: BigString): Promise<GuildPreview> {
    return await this.get(GUILD_PREVIEW(guildID)).then(
      (data) => new GuildPreview(data, this)
    );
  }

  /** Get a guild template */
  async getGuildTemplate(code: string): Promise<GuildTemplate> {
    return await this.get(GUILD_TEMPLATE(code)).then(
      (template) => new GuildTemplate(template, this)
    );
  }
  ...
  async getInvite(inviteID: string, withCounts?: boolean): Promise<Invite> {
    ...
    return await this.get(INVITE(inviteID) + (qs ? "?" + qs : "")).then(
  ...
  async getJoinedPrivateArchivedThreads(
    ...
    return await this.get(
      THREADS_ARCHIVED_JOINED(channelID) + (qs ? "?" + qs : "")
    ).then((response) => {
      return {
        hasMore: response.has_more,
        members: response.members.map(
  ...
  async getMessage(
    ...
    return await this.get(CHANNEL_MESSAGE(channelID, messageID)).then(
      (message) => new Message(message, this)
    );
  }
  ...
  async getMessageReaction(
    ...
    return await this.get(
      CHANNEL_MESSAGE_REACTION(channelID, messageID, reaction) +
        (qs ? "?" + qs : "")
    ...
    if (limit && limit > 100) {
      ...
      const get: (
        ...
        const messages = await this.get(
          CHANNEL_MESSAGES(channelID) + (qs ? "?" + qs : "")
        );
        ...
        logs = _after
          ...
          : logs.concat(
              messages.map(
                (message: DiscordMessage) => new Message(message, this)
              )
            );
  ...
  async getPruneCount(
    ...
    return await this.get(GUILD_PRUNE(guildID) + (qs ? "?" + qs : "")).then(
      (data) => data.pruned
    );
  }
  ...
  async getRESTGuild(guildID: BigString, withCounts = false): Promise<Guild> {
    ...
    return await this.get(GUILD(guildID) + (qs ? "?" + qs : "")).then(
      (guild) => new Guild(guild, this)
    );
  }
  ...
  async getRESTGuildChannels(guildID: BigString): Promise<AnyGuildChannel[]> {
    return await this.get(GUILD_CHANNELS(guildID)).then((channels) =>
      channels.map((channel: DiscordChannel) => Channel.from(channel, this))
    );
  }

  /** Get a guild emoji via the REST API. */
  async getRESTGuildEmoji(
    guildID: BigString,
    emojiID: BigString
  ): Promise<Emoji> {
  ...
  async getRESTGuildMembers(
    ...
    return await this.get(GUILD_MEMBERS(guildID) + (qs ? "?" + qs : "")).then(
      (members) =>
        members.map(
          (member: DiscordMemberWithUser) =>
            new Member(member, this.guilds.get(guildID)!, this)
  ...
  async getRESTGuildRoles(guildID: BigString): Promise<Role[]> {
    return await this.get(GUILD_ROLES(guildID)).then((roles) =>
      roles.map(
        (role: DiscordRole) => new Role(role, this.guilds.get(guildID)!)
      )
    );
  }
  ...
  async getRESTGuilds(options: GetRESTGuildsOptions = {}) {
    ...
    return await this.get(USER_GUILDS("@me") + (qs ? "?" + qs : "")).then(
      (guilds) => guilds.map((guild: DiscordGuild) => new Guild(guild, this))
    );
  }

  /** Get a guild sticker via the REST API. */
  async getRESTGuildSticker(
    guildID: BigString,
    stickerID: BigString
  ): Promise<Sticker> {
  ...
  async getSelf(): Promise<ExtendedUser> {
    return await this.get(USER("@me")).then(
      (data) => new ExtendedUser(data, this)
    );
  }
  ...
  async getStageInstance(channelID: BigString): Promise<StageInstance> {
    return await this.get(STAGE_INSTANCE(channelID)).then(
      (instance) => new StageInstance(instance, this)
    );
  }
  ...
  async getVoiceRegions(guildID: BigString): Promise<VoiceRegion[]> {
    return guildID
      ? await this.get(GUILD_VOICE_REGIONS(guildID))
      : await this.get(VOICE_REGIONS);
  }
  ...
  async getWebhookMessage(
    ...
    return await this.get(WEBHOOK_MESSAGE(webhookID, token, messageID)).then(
      (message) => new Message(message, this)
    );
  }
  ...
  async searchGuildMembers(
    ...
    return await this.get(GUILD_MEMBERS_SEARCH(guildID) + qs).then(
      (members) => {
        const guild = this.guilds.get(guildID);
        return members.map(
          (member: DiscordMemberWithUser) => new Member(member, guild!, this)
        );
      }
    );
  }
  ...
  async syncGuildTemplate(
    ...
    return await this.put(GUILD_TEMPLATE_GUILD(guildID, code)).then(
      (template) => new GuildTemplate(template, this)
    );
  }
  ...
  hasProperty<T extends {}, Y extends PropertyKey = string, Z = unknown>(
    obj: T,
    prop: Y
  ): obj is T & Record<Y, Z> {
  ...
  isDiscordMemberWithUser(
    member: DiscordMember | DiscordMemberWithUser
  ): member is DiscordMemberWithUser {
