...
export class GuildAuditLogEntry extends Base {
  ...
  before: Record<
    DiscordAuditLogChange["key"],
    DiscordAuditLogChange["old_value"]
  >;
  ...
  after: Record<
    DiscordAuditLogChange["key"],
    DiscordAuditLogChange["new_value"]
  >;
      ...
      if (data.options.channel_id) {
        ...
        if (data.options.message_id) {
          this.message = (this.channel &&
            (this.channel as TextChannel).messages.get(data.options.message_id)) || {
  ...
  get target() {
    ...
    if (this.actionType < 10) {
      ...
      // Member
      if (
        this.actionType === AuditLogEvents.MemberMove ||
        this.actionType === AuditLogEvents.MemberDisconnect
      ) {
      ...
      // Message
      return this.guild && this.guild.client.users.get(this.targetID!);
      ...
      // Sticker
      return this.guild?.stickers?.find(
