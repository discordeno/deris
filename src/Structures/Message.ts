...
export class Message extends Base {
  ...
  constructor(data: DiscordMessage, client: Client) {
    ...
    if (data.interaction) {
      ...
      if (data.interaction.member) {
        ...
        this.interaction.member = this.guild.members.get(
      ...
      for (const mention of data.mentions ?? []) {
        ...
        if (mention.member && this.guild) {
          ...
          this.guild.members.set(
            mention.id,
            new Member(
              mention.member as DiscordMemberWithUser,
              this.guild,
              this.client
            )
          );
  ...
  get channelMentions(): string[] {
    return (this.content.match(/<#[0-9]+>/g) || []).map((mention) =>
  ...
  get jumpLink() {
    return `${this.client.CLIENT_URL}${MESSAGE_LINK(
      this.guildID || "@me",
      this.channel.id,
      this.id
    )}`;
  ...
  async createThreadWithMessage(options: {
    ...
    return await this.client.createThreadWithMessage.call(
      this.client,
      this.channel.id,
      this.id,
      options
  ...
  async deleteWebhook(token: string): Promise<void> {
    ...
    return await this.client.deleteWebhookMessage.call(
      this.client,
      this.webhookID,
      token,
      this.id
  ...
  async pin(): Promise<void> {
    ...
    return await this.client.pinMessage.call(
      this.client,
      this.channel.id,
      this.id
    );
  ...
  async removeReactionEmoji(reaction: string): Promise<void> {
    ...
    return await this.client.removeMessageReactionEmoji.call(
      this.client,
      this.channel.id,
      this.id,
      reaction
