...
export class Invite {
  ...
  constructor(data: DiscordInvite | DiscordInviteCreate, client: Client) {
    ...
    if (this.isInviteCreate(data)) {
      ...
      if (data.guild) {
        if (client.guilds.has(data.guild.id!)) {
          if (data.channel) {
            // @ts-ignore should work i think dumb partials
    ...
    this.targetApplicationID =
      data.target_application !== undefined ? data.target_application.id : null;
    this.targetType = data.target_type !== undefined ? data.target_type : null;
    this.targetUser =
      data.target_user !== undefined
        ? new User(data.target_user, client)
        : null;
  ...
  isInviteCreate(
    data: DiscordInvite | DiscordInviteCreate
  ): data is DiscordInviteCreate {
