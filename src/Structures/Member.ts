...
import { MemberOptions } from "../typings.js";
...
export class Member extends Base {
  ...
  constructor(
    data: (DiscordMember & { id: BigString }) | DiscordMemberWithUser,
    guild: Guild,
    client: Client
  ) {
    ...
    const userID = client.isDiscordMemberWithUser(data)
      ? data.user.id
      : data.id;
  ...
  update(data: (DiscordMember & { id: BigString }) | DiscordMemberWithUser) {
    ...
    if (data.avatar !== undefined)
      this._avatar = this.client.iconHashToBigInt(data.avatar);
  ...
  get avatar(): string | undefined {
    return this._avatar
      ? this.client.iconBigintToHash(this._avatar)
      : undefined;
  }
  ...
  get avatarURL() {
    return this.avatar
      ? this.client._formatImage(
          GUILD_AVATAR(this.guild.id, this.id, this.avatar)
        )
      : this.user.avatarURL;
  }
